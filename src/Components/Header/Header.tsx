...
import { Fab, MenuItem, Select, SelectChangeEvent, TextField } from '@mui/material'
...
const Header = (): JSX.Element => {
    ...
    return (
        <header className='flex flex-col bg-main'>
            <div className='justify-center bg-header flex-col sm:flex-row card_header flex top-0 lg:gap-4 lg:justify-between items-center border-b-2'>
                <div className='fixed bottom-9 left-9 sm:static lg:hidden flex flex-1 justify-center items-center container-menu'>
                    <Fab color="primary" id="menu" className='' >
                        <button className='text-4xl' onClick={() => {
                            document.querySelector('#navigate')?.classList.add('displayNav')
                            document.querySelector("body")?.classList.add("before")
                        }}><IoMenu /></button>
                    </Fab>
                    <button className='text-4xl hidden sm:block' onClick={() => {
                        document.querySelector('#navigate')?.classList.add('displayNav')
                        document.querySelector("body")?.classList.add("before")
                    }}><IoMenu /></button>
                </div>
                <img src={logo} alt="Logo Blue Health" className=' h-44 cursor-pointer flex-1' onClick={() => navigate('/')} />
                ...
                <div className='flex flex-wrap items-center !justify-center text-gray-50 !pb-5 sm:!pb-0 flex-1 gap-2 '>
                    <button className='text-2xl sm:text-2xl px-3 flex justify-center items-center gap-3  text-black hover:text-navBar lg:hidden' onClick={() => {
                        document.querySelector("#form")?.classList.add("displayMenu");
                        (document.querySelector("#search") as HTMLInputElement).focus()
                    }}> <CiSearch className='text-4xl' /> </button>
                    <button className='text-2xl sm:text-2xl px-3 flex !justify-center items-center gap-3  text-black hover:text-navBar' onClick={() => navigate('/login')} >
                        <CiUser className='text-4xl' /> <span className='hidden lg:block'>Cuenta</span></button>
                    <button onClick={activateCar} className=' px-3 text-2xl justify-center sm:text-2xl relative gap-3 text-black flex'>
                        <CiShoppingCart className='self-center text-4xl' /> <span className='hidden lg:block self-center'>Cesta</span></button>
                </div>
            </div>
        </header>
    )
